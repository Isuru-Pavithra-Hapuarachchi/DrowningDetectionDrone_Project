...
def video_feed():
    ...
    while not stop_detection:
        ...
        if detection_counter >= 5:
            # send_telegram_message("Drowning person detected in live feed!")
            detection_counter = 0
            stop_detection = True
...
def process_video(file_path):
    ...
    while cap.isOpened():
        ...
        if detection_counter >= 5:
            # send_telegram_message("Drowning person detected in video!")
            detection_counter = 0
...
def emergency_land():
    ...
    stop_detection = True
    tello.land()
    messagebox.showinfo("Emergency Land", "Drone has landed immediately!")
...
emergency_land_button = tk.Button(button_frame, text="Emergency Land", command=emergency_land, bg='#FF0000', fg='white', font=('Helvetica', 12, 'bold'), width=25, height=2, relief='raised', bd=5)
emergency_land_button.pack(pady=10)
