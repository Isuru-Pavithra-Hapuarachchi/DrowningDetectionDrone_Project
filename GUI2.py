...
model = torch.hub.load('ultralytics/yolov5', 'custom', path=r'C:\Project\Project\yolov5\runs\train\exp2\weights\best.pt')
...
# Create a frame for logo and title
header_frame = tk.Frame(root, bg='#f0f0f0')
header_frame.pack(fill='x', padx=10, pady=10)

# Load and display the logo
logo_path = r'C:\Project\Project\yolov5\logo.jpeg'
logo_img = Image.open(logo_path)
logo_img = logo_img.resize((100, 100), Image.LANCZOS)  # Resize the logo if needed
logo_tk = ImageTk.PhotoImage(logo_img)
logo_label = tk.Label(header_frame, image=logo_tk, bg='#f0f0f0')
logo_label.image = logo_tk  # Keep a reference to avoid garbage collection
logo_label.pack(side='left', padx=10)

# Create a heading label next to the logo
heading_label = tk.Label(header_frame, text="Drowning Detection System", font=('Helvetica', 18, 'bold'), bg='#f0f0f0')
heading_label.pack(side='left', padx=10)

# Create frames for buttons and video feed
top_buttons_frame = tk.Frame(root, bg='#f0f0f0')
top_buttons_frame.pack(side='right', padx=10, pady=10)
...
# Create and place the top-right section buttons with styles
top_button_style = {
    'fg': 'white',
...
connect_button = tk.Button(top_buttons_frame, text="Connect Drone", command=connect_drone, **top_button_style)
connect_button.grid(row=0, column=0, padx=10, pady=5)

fly_button = tk.Button(top_buttons_frame, text="Start and Fly Drone", command=start_fly_drone, **top_button_style)
fly_button.grid(row=1, column=0, padx=10, pady=5)

start_video_button = tk.Button(top_buttons_frame, text="Start Video Detection", command=start_video_detection, **top_button_style)
start_video_button.grid(row=2, column=0, padx=10, pady=5)

emergency_button = tk.Button(top_buttons_frame, text="Emergency Land", command=emergency_land, bg='red', **top_button_style)


# Create and place the other buttons below the top-right section
other_button_style = {
    'bg': '#4CAF50',
    'fg': 'white',
    'font': ('Helvetica', 12, 'bold'),
...
stop_video_button = tk.Button(button_frame, text="Stop Video Detection", command=stop_video_detection, **other_button_style)
stop_video_button.pack(pady=10)

upload_video_button = tk.Button(button_frame, text="Upload and Process Video", command=upload_and_process_video, **other_button_style)
upload_video_button.pack(pady=10)

add_user_button = tk.Button(button_frame, text="Add Telegram User", command=add_new_telegram_user, **other_button_style)
add_user_button.pack(pady=10)

disconnect_button = tk.Button(button_frame, text="Disconnect Drone", command=disconnect_drone, **other_button_style)
