...
def get_drone_position():
    # Placeholder for getting actual drone position
...
def video_feed():
    ...
    while not stop_detection:
        ...
        if detection_counter >= 5:
            ...
            send_telegram_message(f"Drowning person detected in live feed! Drone Position - X: {x}, Y: {y}, Z: {z}")
            detection_counter = 0
            stop_detection = True
...
upload_button = tk.Button(button_frame, text="Upload and Process Video", command=upload_and_process_video, **button_style)
